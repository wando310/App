...
import ListPizza from '../../../assets/pizzas'
import pizzaArray from '../../../assets/pizzas/pizzaArray';

import FooterCompenent from '../../components/footerPageComponent'

import ModalMenuList from '../../components/modalMenuList'

export default function ListPizzaMenu({navigation}) {  
    ...
    const [list, setList] = useState(pizzaArray)
    useEffect(()=>{
        if(search === ''){
            setList(pizzaArray)
        }else{
            setList(
                pizzaArray.filter( item =>{
