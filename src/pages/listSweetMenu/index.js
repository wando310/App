...
import ListSweet from '../../../assets/doces'
import sweetArray from '../../../assets/doces/sweetArray'

import FooterCompenent from '../../components/footerPageComponent'

import ModalMenuList from '../../components/modalMenuList'

export default function ListSweetMenu({ navigation }) {  
    ...
    const [list, setList] = useState(sweetArray)
    useEffect(() => {
        if (search === '') {
            setList(sweetArray)
        } else {
            setList(
                sweetArray.filter(item => {
